...
class AttentionUpdate(nn.Module):
    ...
    def _attention_chunk(self, query, key, value):
        # This is exact scaled dot-product attention for one subset of query
        # tokens. Every query in the subset still attends to every key/value.
    ...
    def forward(self, query, context):
        ...
        # Split only the query dimension to avoid constructing the complete
        # L_q x L_k attention matrix at once. Checkpointing recomputes each
        # chunk during backward instead of retaining all chunk matrices.
        attended_chunks = []
        for query_chunk in torch.split(
            ...
            if self.training:
                ...
                attended_chunk = self._attention_chunk(
                    query_chunk,
                    projected_key,
                    projected_value)
